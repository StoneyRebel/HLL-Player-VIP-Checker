...
class CRCONService {
    ...
    async getPlayerByT17Username(t17Username) {
        Logger.info(`🔍 ENHANCED DEBUG: Starting search for "${t17Username}"`);
        try {
            ...
            for (const strategy of searchStrategies) {
                try {
                    ...
                    Logger.info(`🔍 Testing ${strategy.name} for ${t17Username}`);
                    ...
                    if (player) {
                        Logger.info(`✅ FOUND via ${strategy.name}:`, JSON.stringify(player));
